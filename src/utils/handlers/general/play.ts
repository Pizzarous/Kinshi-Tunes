			...
			: setTimeout(() => {
				...
				void queue.textChannel
					...
					.then(msg => {
						...
						console.log("Time out finished, see you later! UwU")
					});
			}, 1800000);

		console.log("Time out started, leaving in half an hour if no one talks to me >:C")
	...
	async function playResource(): Promise<void> {
		if (guild.channels.cache.get(queue!.connection!.joinConfig.channelId!)?.type === "GUILD_STAGE_VOICE") {
			queue?.client.debugLog.logData(
				"info",
				"PLAY_HANDLER",
				`Trying to be a speaker in ${guild.me?.voice.channel?.name ?? "Unknown"}(${guild.me?.voice.channel?.id ?? "ID UNKNOWN"
			...
			if (suppressed && "error" in suppressed) {
				queue?.client.debugLog.logData(
					"error",
					"PLAY_HANDLER",
					`Failed to be a speaker in ${guild.me?.voice.channel?.name ?? "Unknown"}(${guild.me?.voice.channel?.id ?? "ID UNKNOWN"
